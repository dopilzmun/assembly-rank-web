...
### 🗂️ RankingDashboard.tsx 랭킹 대시보드 컴포넌트
* **원터치 가로 스크롤 정당 칩 필터 (`PARTY_CHIP_ACTIVE`):** 전체 및 각 정당별 고유 컬러 칩 버튼으로 모바일 터치 효율성 극대화.
* **뷰 모드 토글 (Card ⇄ Table):** 모바일 기본(컴팩트 카드), 데스크톱 기본(테이블) 및 수동 토글 지원.
* **모바일 네비게이션 겹침 방지 맞비교 독 (`bottom-16 sm:bottom-5`):** 모바일 하단 플로팅 탭바와 충돌하지 않도록 안전 마진 확보.
* **통일된 100점 만점 산식 배너:** 홈 화면과 일치된 가결45+상정35+발의20 컬러 코딩 칩 적용.

---

### 🗂️ LegislativeLiveRadar.tsx (실시간 뉴스룸 & 파이프라인 레이더)
* **모바일 피드 3단 계층화:**
  - 1단: 날짜 + 액션 뱃지 + 소관 상임위 및 가결/상정 결과 텍스트
  - 2단: 법안명 말줄임 및 전체 툴팁
  - 3단: 의원 상세 성적표 연동 마이크로 프로필 칩 + HTTPS 공식 의안 원문 아웃링크
* **가결 안건 메타데이터 보강:** `b.curr_cmit_nm`을 조인하여 `[소관위 · 대안반영 가결]` 형태로 본회의 통과 법안의 관할 위원회를 명확히 표시.

---

### 🗂️ CommitteeBottleneckSection.tsx (상임위 병목 진단 섹션)
* **목록 기본 펼침 (`isExpanded: true`):** 클릭 없이 진입 즉시 17개 상임위 데이터 브라우징 지원.
* **원터치 퀵 정렬 칩 바:** 접수량순, ⚡ 신속 착수순, 🚨 병목 지연순, 상정 진행도순, 가결률순 원터치 토글.
* **소속 의원 순위표 연계 링크:** 각 상임위별 `/rankings?q={curr_cmit_nm}` 바로가기 제공.
* **다크모드 지원 및 신호등 뱃지 정규화:** 원활(Emerald), 보통(Amber), 정체(Rose) 상태 칩 스타일 최적화.

---

### ⚡ 인프라 및 서버리스 최적화 설정
- **런타임 리전 고정 (`vercel.json`):** `icn1` (서울/인천) 지정으로 DB와의 물리적 통신 지연(RTT) 5ms 이하 단축.
- **서버리스 커넥션 풀링 (`src/lib/db.ts`):** `connectionLimit: 2`, `idleTimeout: 30000`, `globalThis` 풀 캐싱으로 서버리스 스케일아웃 시 커넥션 고갈 원천 차단.
- **홈 화면 ISR 캐싱 (`src/app/page.tsx`):** `revalidate = 3600` (1시간) 전환으로 메인 화면 DB 조회 부하 99.9% 소멸.
...
* **2026-09-15 (Phase 2-1 Rankings 대시보드 모바일 UX 및 플로팅 독 간섭 해소):**
  - 맞비교 플로팅 독의 위치를 `bottom-16 sm:bottom-5`로 조정하여 모바일 하단 네비게이션과의 겹침 현상 원천 차단.
  - 가로 스크롤 원터치 정당 칩 필터 및 카드/테이블 뷰 전환 토글 신설.
  - 100점 만점 산식 배너 칩 디자인을 홈 화면과 일치화.
* **2026-09-15 (Phase 2-2 /live 실시간 입법 피드 모바일 UI/UX 고도화):**
  - 가결 안건에 소관 상임위 메타데이터 결합 표기.
  - 타임라인 카드를 3단 계층(메타/법안명/의원칩+원문링크)으로 개편하여 모바일 터치 사용성 및 가독성 개선.
  - LIKMS 외부 링크를 HTTPS 프로토콜로 전환하고 전체 다크모드 대응 완료.
* **2026-09-15 (Phase 2-2 /committees 상임위 심사 속도 & 병목 진단 고도화):**
  - 상임위 목록 기본 노출 전환 및 5대 퀵 정렬 칩 탑재.
  - 상임위 카드 및 행에서 소속 국회의원 순위표(`/rankings?q=...`)로 즉시 이동하는 연계 네비게이션 구축.
  - 전체 다크모드 대응 및 신호등 상태 뱃지 시인성 강화.
* **2026-09-15 (서버리스 인프라 레이턴시 및 DB 커넥션 안정화):**
