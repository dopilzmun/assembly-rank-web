...
import { Heart, Calendar, ExternalLink, Tag } from "lucide-react";
...
const CATEGORIES = [
  { key: "ALL", label: "전체" },
  { key: "WORK", label: "직장·노동" },
  { key: "HOUSE", label: "주거·부동산" },
  { key: "CARE", label: "육아·돌봄" },
  { key: "FIN", label: "금융·경제" },
  { key: "TRAF", label: "교통·이동" },
  { key: "LIFE", label: "생활·안전" },
];

// 분야별 한글 명칭 및 뱃지 컬러 매핑
...
> = {
  WORK: {
    label: "직장·노동",
    badgeStyle: "bg-blue-100 text-blue-800 dark:bg-blue-950/70 dark:text-blue-300",
  },
  HOUSE: {
    label: "주거·부동산",
    badgeStyle: "bg-amber-100 text-amber-800 dark:bg-amber-950/70 dark:text-amber-300",
  },
  CARE: {
    label: "육아·돌봄",
    badgeStyle: "bg-pink-100 text-pink-800 dark:bg-pink-950/70 dark:text-pink-300",
  },
  FIN: {
    label: "금융·경제",
    badgeStyle: "bg-emerald-100 text-emerald-800 dark:bg-emerald-950/70 dark:text-emerald-300",
  },
  TRAF: {
    label: "교통·이동",
    badgeStyle: "bg-purple-100 text-purple-800 dark:bg-purple-950/70 dark:text-purple-300",
  },
  LIFE: {
    label: "생활·안전",
    badgeStyle: "bg-teal-100 text-teal-800 dark:bg-teal-950/70 dark:text-teal-300",
  },
};
  ...
  useEffect(() => {
    ...
    const url =
      selectedCategory === "ALL"
        ...
        : `/api/district/life-changes?ctgr_se=${selectedCategory}`;
    ...
    <div className="rounded-2xl border border-slate-200/80 bg-white p-6 shadow-xs dark:border-slate-800 dark:bg-slate-900 transition-all space-y-6">
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          {changes.map((item) => {
            const catConfig = CATEGORY_CONFIG[item.ctgr_se] || {
              label: item.ctgr_se,
              badgeStyle: "bg-slate-100 text-slate-800 dark:bg-slate-800 dark:text-slate-300",
            };
            ...
            return (
              ...
              >
                <div className="space-y-3.5">
                  {/* 카드 헤더: 한글 표준 카테고리 뱃지 + 적용 대상 + 시행일 */}
                  <div className="flex items-center justify-between gap-2 text-xs">
                    <div className="flex items-center gap-1.5 flex-wrap">
                      <span className={`font-bold px-2.5 py-0.5 rounded-md text-[11px] ${catConfig.badgeStyle}`}>
                        {catConfig.label}
                      </span>
                      {item.tgt_cnts && (
