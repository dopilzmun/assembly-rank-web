          ...
          sortedData.map((row) => {
            ...
            return (
              <div
                ...
                className={`bg-white rounded-xl border p-3.5 shadow-sm transition-all active:scale-[0.99] cursor-pointer ${
              ...
              >
                ...
                <div className="flex items-center justify-between pb-2 mb-2 border-b border-slate-100">
                  <div className="flex items-center gap-1.5 truncate">
                    {isDeferred ? (
                      <span className="px-1.5 py-0.2 rounded text-[10px] font-bold bg-slate-100 text-slate-500 shrink-0">
                        유예
                      </span>
                    ) : row.rnkg && row.rnkg <= 3 ? (
                      <span className="w-5 h-5 flex items-center justify-center bg-indigo-600 text-white rounded-full font-black text-xs shrink-0">
                        {row.rnkg}
                      </span>
                    ) : (
                      <span className="text-xs font-bold font-mono text-slate-500 min-w-[16px] text-center shrink-0">
                        {row.rnkg ?? "-"}
                      </span>
                    )}
                    ...
                    {tag && (
                      <span className={`px-1.5 py-0.2 rounded text-[9px] font-semibold border whitespace-nowrap ${tag.style}`}>
                        {tag.label}
                      </span>
                    )}
                  </div>
                ...
                <div className="grid grid-cols-4 gap-1 text-center bg-slate-50/80 rounded-lg p-2 font-mono">
                  <div className="overflow-hidden">
                    ...
                    <strong className="text-xs font-bold text-slate-800 whitespace-nowrap">
                  ...
                  <div className="overflow-hidden">
                    <span className="text-[10px] text-slate-400 block whitespace-nowrap truncate">상임위</span>
                    <strong className="text-xs font-bold text-blue-700 whitespace-nowrap">
                  ...
                  <div className="overflow-hidden">
                    <span className="text-[10px] text-slate-400 block whitespace-nowrap truncate">상정률</span>
                    <strong className="text-xs font-bold text-slate-800 whitespace-nowrap">
                  ...
                  <div className="overflow-hidden">
                    <span className="text-[10px] text-slate-400 block whitespace-nowrap truncate">실질가결</span>
                    <strong className="text-xs font-bold text-emerald-600 whitespace-nowrap">
                      {Number(row.aprv_cnt) || 0}건
                    </strong>
                    <span className="text-[9px] text-slate-500 block font-sans whitespace-nowrap">
                      원{pureCnt}·대{altCnt}
                    </span>
                  </div>
                </div>
        ...
        <div className="fixed bottom-4 left-3 right-3 md:left-1/2 md:right-auto md:-translate-x-1/2 z-40 bg-slate-900 text-white px-3.5 py-2.5 rounded-2xl shadow-2xl border border-slate-700 flex items-center justify-between md:justify-start gap-2 sm:gap-3 animate-in fade-in slide-in-from-bottom-3 duration-200">
          ...
          <div className="flex items-center gap-1.5 text-xs truncate">
            {compareList.map((m) => (
              <span
                key={m.assemb_id}
                className="inline-flex items-center gap-1 px-2 py-0.5 bg-slate-800 rounded-lg border border-slate-700 font-semibold text-[11px] sm:text-xs shrink-0 whitespace-nowrap"
              >
                {m.assemb_nm}
            ...
            {compareList.length === 1 && (
              <button
                ...
                className="inline-flex items-center gap-1 px-2 py-0.5 bg-indigo-600/30 hover:bg-indigo-600/50 border border-indigo-400/40 rounded-lg text-indigo-300 font-medium text-[11px] whitespace-nowrap animate-pulse transition-colors"
              >
