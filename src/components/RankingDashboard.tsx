...
const PARTY_CHIP_ACTIVE: Record<string, string> = {
  ALL: "bg-slate-900 text-white dark:bg-slate-100 dark:text-slate-900 shadow-xs",
  더불어민주당: "bg-blue-600 text-white shadow-xs",
  국민의힘: "bg-red-600 text-white shadow-xs",
  조국혁신당: "bg-sky-600 text-white shadow-xs",
  개혁신당: "bg-orange-600 text-white shadow-xs",
  진보당: "bg-purple-600 text-white shadow-xs",
  기본소득당: "bg-teal-600 text-white shadow-xs",
  사회민주당: "bg-yellow-600 text-white shadow-xs",
  무소속: "bg-gray-700 text-white shadow-xs",
};
...
function getLegislativeTag(row: BillRankingRow) {
  if (row.is_deferred === 1) {
    return {
      label: "유예",
      style: "bg-slate-100 text-slate-600 border-slate-300 dark:bg-slate-800 dark:text-slate-400",
  ...
  if (motnCnt === 0) {
    return {
      label: "특수",
      style: "bg-slate-100 text-slate-700 border-slate-200 dark:bg-slate-800 dark:text-slate-400",
...
export default function RankingDashboard({ initialData, weeklyRadar }: RankingDashboardProps) {
  ...
  // 카드 뷰 / 테이블 뷰 표시 여부 결정
  const showCards = viewMode === "card" || (viewMode === "auto");
  ...
  return (
    <div className="space-y-6">
      ...
      {/* 1. 검색 및 다차원 필터 컨트롤 패널 */}
      <div className="bg-white rounded-2xl shadow-xs border border-slate-200/90 p-4 sm:p-5 space-y-3 sm:space-y-4 dark:bg-slate-900 dark:border-slate-800">
        ...
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-3">
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2.5 text-xs sm:text-sm border border-slate-200 rounded-xl focus:outline-hidden focus:ring-2 focus:ring-indigo-500 transition-all font-medium dark:border-slate-700 dark:bg-slate-800 dark:text-slate-100"
            />
          </div>
          ...
          <div className="flex items-center gap-2">
            <select
              ...
              className="w-full sm:w-auto px-3 py-2 text-xs sm:text-sm border border-slate-200 rounded-xl bg-white text-slate-700 font-medium focus:ring-2 focus:ring-indigo-500 max-w-[200px] dark:border-slate-700 dark:bg-slate-800 dark:text-slate-300"
            >
            ...
            {isFilterActive && (
              <button
                ...
                className="inline-flex items-center justify-center gap-1 px-3 py-2 text-xs sm:text-sm font-semibold text-slate-600 hover:text-slate-900 bg-slate-100 hover:bg-slate-200 rounded-xl transition-colors cursor-pointer shrink-0 dark:bg-slate-800 dark:text-slate-300 dark:hover:bg-slate-700"
              >
        ...
        {/* 1-2. 원터치 정당 스크롤 칩 (Pill/Chip) 필터 */}
        <div className="flex items-center gap-1.5 overflow-x-auto pb-1 scrollbar-none pt-1">
          <span className="text-xs font-bold text-slate-400 shrink-0 mr-1">정당:</span>
          
          <button
            onClick={() => setSelectedParty("ALL")}
            className={`px-3 py-1 rounded-full text-xs font-bold transition-all shrink-0 cursor-pointer ${
              selectedParty === "ALL"
                ? PARTY_CHIP_ACTIVE["ALL"]
                : "bg-slate-100 text-slate-600 hover:bg-slate-200 dark:bg-slate-800 dark:text-slate-400"
            }`}
          >
            전체 ({initialData.length})
          </button>

          {partyList.map((party) => {
            const isSelected = selectedParty === party;
            const count = initialData.filter((m) => m.pltprt_nm === party).length;
            return (
              <button
                key={party}
                ...
                className={`px-3 py-1 rounded-full text-xs font-bold transition-all shrink-0 cursor-pointer ${
                  isSelected
                    ? PARTY_CHIP_ACTIVE[party] || "bg-indigo-600 text-white"
                    : "bg-slate-100 text-slate-600 hover:bg-slate-200 dark:bg-slate-800 dark:text-slate-400"
                }`}
              >
                {party} ({count})
        ...
        {/* 1-3. 발의 건수 허들 및 뷰 모드 토글 바 */}
        <div className="flex flex-wrap items-center justify-between gap-3 pt-3 border-t border-slate-100 dark:border-slate-800 text-xs sm:text-sm">
          
          {/* 최소 발의 허들 칩 */}
          <div className="flex items-center gap-1.5 overflow-x-auto scrollbar-none">
            <span className="font-semibold text-slate-400 shrink-0">최소 발의:</span>
            {[
              { label: "전체", value: 0 },
              { label: "5건+", value: 5 },
              { label: "10건+", value: 10 },
          ...
          {/* 우측 도구: 정렬 드롭다운 & 뷰 모드 토글 (카드 vs 테이블) */}
          <div className="flex items-center gap-2 ml-auto">
            ...
            <select
              ...
              className="text-xs bg-slate-100 border border-slate-200 rounded-xl px-2.5 py-1.5 font-bold text-slate-700 dark:bg-slate-800 dark:border-slate-700 dark:text-slate-300"
            >
            ...
            {/* 뷰 모드 토글 버튼 (카드 vs 표) */}
            <div className="flex items-center bg-slate-100 p-0.5 rounded-xl dark:bg-slate-800 border border-slate-200 dark:border-slate-700">
              <button
      ...
      {/* 2. 종합점수 100점 산식 안내 배너 (홈 화면 Zone 2와 통일) */}
      <div className="rounded-2xl border border-indigo-200/80 bg-gradient-to-r from-indigo-50/80 via-white to-indigo-50/40 p-4 dark:border-indigo-950/80 dark:from-slate-900 dark:via-slate-900 dark:to-indigo-950/40 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 shadow-xs">
        <div className="space-y-0.5">
          <div className="flex items-center gap-1.5 text-indigo-700 dark:text-indigo-300 font-bold text-xs sm:text-sm">
            <Scale className="w-4 h-4 text-indigo-600 shrink-0" />
            <span>왜곡 없는 입법 효율성 100점 만점 평가 산식</span>
          </div>
          <p className="text-[11px] text-slate-500 dark:text-slate-400">
            단순 발의 수 과열을 방지하고 본회의 실질가결(원안 1.0 + 대안 0.7)과 상임위 심사착수에 높은 가중치를 둡니다.
          </p>
        </div>

        <div className="flex items-center gap-1.5 font-mono text-xs font-bold shrink-0 flex-wrap">
          <span className="px-2 py-0.5 rounded-md bg-white border border-emerald-200 text-emerald-700 shadow-xs dark:bg-slate-800 dark:border-emerald-900 dark:text-emerald-300">
            가결 45
          </span>
          <span className="text-slate-400">+</span>
          <span className="px-2 py-0.5 rounded-md bg-white border border-indigo-200 text-indigo-700 shadow-xs dark:bg-slate-800 dark:border-indigo-900 dark:text-indigo-300">
            상정 35
          </span>
          <span className="text-slate-400">+</span>
          <span className="px-2 py-0.5 rounded-md bg-white border border-slate-200 text-slate-700 shadow-xs dark:bg-slate-800 dark:border-slate-700 dark:text-slate-300">
            발의 20
          </span>
        </div>
      </div>
      ...
      <div className="flex items-center justify-between text-xs sm:text-sm text-slate-500 px-1">
        <div>
          조회 결과: <strong className="text-slate-900 dark:text-slate-100 font-black font-mono">{sortedData.length}</strong>명 / 전체 {initialData.length}명
        </div>
        <span className="text-slate-400 text-xs hidden sm:inline">
          * 의원 행이나 카드를 클릭하면 상세 성적표 드로어가 열립니다.
        </span>
      </div>

      {/* 3. [카드 뷰] (모바일 기본 활성화 & 토글 선택 시 노출) */}
      {(viewMode === "card" || (viewMode === "auto" && showCards)) && (
        <div className={`space-y-3 ${viewMode === "auto" ? "block md:hidden" : "block"}`}>
          {sortedData.length > 0 ? (
            sortedData.map((row) => {
              ...
              return (
                <div
                  ...
                  className={`bg-white rounded-2xl border p-4 shadow-xs transition-all active:scale-[0.99] cursor-pointer space-y-3 dark:bg-slate-900 ${
                    isSelectedForCompare
                      ? "border-indigo-500 ring-2 ring-indigo-500/20 bg-indigo-50/20 dark:border-indigo-500"
                ...
                >
                  ...
                  <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-2.5">
                    <div className="flex items-center gap-2 truncate">
                      {isDeferred ? (
                        <span className="px-2 py-0.5 rounded-md text-xs font-bold bg-slate-100 text-slate-500 shrink-0 dark:bg-slate-800 dark:text-slate-400">
                          유예
                        </span>
                      ) : row.rnkg && row.rnkg <= 3 ? (
                        ...
                        <span className="text-sm font-bold font-mono text-slate-500 min-w-[20px] text-center shrink-0 dark:text-slate-400">
                          {row.rnkg ?? "-"}
                        </span>
                      )}

                      <strong className="font-extrabold text-slate-900 text-base whitespace-nowrap dark:text-slate-100">
                        {row.assemb_nm}
                      </strong>
                    ...
                    <div className="flex items-center gap-2 shrink-0" onClick={(e) => e.stopPropagation()}>
                      <span className="font-mono text-sm font-black text-indigo-700 bg-indigo-50 px-2 py-0.5 rounded-lg border border-indigo-100 dark:bg-indigo-950/70 dark:border-indigo-900 dark:text-indigo-300">
                      ...
                      <button
                        ...
                        className={`w-8 h-8 rounded-xl text-xs font-black transition-all flex items-center justify-center shrink-0 cursor-pointer ${
                          isSelectedForCompare
                            ...
                            : "bg-slate-100 text-slate-400 hover:bg-indigo-100 hover:text-indigo-700 dark:bg-slate-800 dark:hover:bg-slate-700"
                        }`}
                      >
                        VS
                      </button>
                  ...
                  {/* 2열: 소속 상임위 및 지역구 */}
                  <div className="flex items-center justify-between text-xs text-slate-500 font-medium px-0.5 dark:text-slate-400">
                    <span className="truncate max-w-[65%]">{row.cmit_nm || "상임위 미배정"}</span>
                    <span className="shrink-0 text-slate-400">{row.rgn_nm || "비례대표"}</span>
                  </div>

                  {/* 3열: 3대 핵심 지표 그리드 */}
                  <div className="grid grid-cols-3 gap-2 bg-slate-50/90 rounded-xl p-2.5 text-center font-mono dark:bg-slate-800/50">
                    <div className="p-1">
                      <span className="text-xs text-slate-500 font-sans block mb-0.5 font-medium dark:text-slate-400">대표발의</span>
                      <strong className="text-sm sm:text-base font-bold text-slate-900 block dark:text-slate-100">
                        {motnCnt}건
                      </strong>
                    ...
                    <div className="p-1 border-x border-slate-200/70 dark:border-slate-700">
                      <span className="text-xs text-slate-500 font-sans block mb-0.5 font-medium dark:text-slate-400">상임위상정</span>
                      <strong className="text-sm sm:text-base font-bold text-indigo-700 block dark:text-indigo-400">
                    ...
                    <div className="p-1">
                      <span className="text-xs text-emerald-700 font-sans block mb-0.5 font-bold dark:text-emerald-400">실질가결</span>
                      <strong className="text-sm sm:text-base font-bold text-emerald-600 block dark:text-emerald-400">
                        {Number(row.aprv_cnt) || 0}건
                      </strong>
                      <span className="text-[11px] text-slate-500 font-sans block dark:text-slate-400">
                        원{pureCnt} · 대{altCnt}
                      </span>
                    </div>
                  </div>
            ...
            <div className="bg-white rounded-2xl border border-slate-200 p-8 text-center text-sm text-slate-400 dark:bg-slate-900 dark:border-slate-800">
              조건에 일치하는 국회의원이 없습니다.
            </div>
          )}
        </div>
      ...
      {/* 4. [테이블 뷰] (데스크톱 기본 활성화 & 토글 선택 시 노출) */}
      {(viewMode === "table" || (viewMode === "auto" && showTable)) && (
        <div className={`bg-white rounded-2xl shadow-xs border border-slate-200 overflow-x-auto dark:bg-slate-900 dark:border-slate-800 ${
          viewMode === "auto" ? "hidden md:block" : "block"
        }`}>
          <table className="w-full text-left text-sm text-slate-600 dark:text-slate-300 table-auto">
            <thead className="bg-slate-100/80 text-slate-700 font-semibold text-xs tracking-wider border-b border-slate-200 select-none whitespace-nowrap dark:bg-slate-800 dark:text-slate-200 dark:border-slate-700">
              <tr>
                ...
                sortedData.map((row) => {
                  ...
                  return (
                    <tr
                      key={row.assemb_id}
                      onClick={() => handleSelectAssemb(row)}
                      className={`hover:bg-indigo-50/50 cursor-pointer transition-colors group dark:hover:bg-slate-800/60 ${
                        isSelectedForCompare ? "bg-indigo-50/60 dark:bg-indigo-950/40" : isDeferred ? "bg-slate-50/40 opacity-75" : ""
                      }`}
                    >
                        ...
                        <button
                          ...
                          className={`w-7 h-7 rounded-lg text-xs font-black transition-all flex items-center justify-center mx-auto cursor-pointer ${
                            isSelectedForCompare
                              ...
                              : "bg-slate-100 text-slate-400 hover:bg-indigo-100 hover:text-indigo-700 dark:bg-slate-800"
                          }`}
                        >
                          VS
                        </button>
                      ...
                      <td className="py-3 px-2 text-center font-bold text-slate-900 whitespace-nowrap dark:text-slate-100">
                        {isDeferred ? (
                          <span className="inline-flex items-center px-1.5 py-0.5 rounded text-xs font-semibold bg-slate-200 text-slate-600 dark:bg-slate-800 dark:text-slate-400">
                            유예
                          </span>
                        ) : row.rnkg && row.rnkg <= 3 ? (
                          <span className="inline-flex items-center justify-center w-6 h-6 bg-indigo-600 text-white rounded-full font-black text-xs shadow-xs mx-auto">
                            {row.rnkg}
                          </span>
                        ) : (
                      ...
                      <td className="py-3 px-2 text-right whitespace-nowrap">
                        {isDeferred || row.score === null ? (
                          ...
                          <span className="px-2 py-0.5 rounded-full text-xs font-black bg-indigo-50 text-indigo-700 border border-indigo-200 font-mono shadow-xs dark:bg-indigo-950 dark:border-indigo-900 dark:text-indigo-300">
                      ...
                      <td className="py-3 px-3">
                        <div className="flex flex-col">
                          <div className="inline-flex items-center gap-1.5 whitespace-nowrap">
                            <span className="font-bold text-slate-900 group-hover:text-indigo-600 transition-colors dark:text-slate-100">
                              {row.assemb_nm}
                            </span>
                            {tag && (
                      ...
                      <td className="py-3 px-2 text-slate-600 text-xs truncate max-w-[100px] lg:max-w-[130px] dark:text-slate-400">
                        {row.rgn_nm || "비례대표"}
                      </td>

                      <td className="py-3 px-2 text-right font-mono whitespace-nowrap">
                        <div className="flex flex-col items-end">
                          <span className="text-slate-800 font-semibold dark:text-slate-200">{motnCnt.toLocaleString()}건</span>
                      ...
                      <td className="hidden xl:table-cell py-3 px-2 text-right font-mono whitespace-nowrap">
                        {Number(row.avg_cmt_days) > 0 ? (
                          <span className="inline-flex items-center gap-1 text-slate-700 text-xs dark:text-slate-300">
                      ...
                      <td className="py-3 px-3 text-right whitespace-nowrap">
                        <div className="flex flex-col items-end">
                          <span className="font-bold text-emerald-600 font-mono dark:text-emerald-400">
                            {(Number(row.aprv_cnt) || 0).toLocaleString()}건
                          </span>
                          <div className="flex items-center gap-1 text-xs text-slate-500 font-mono">
                            <span className="text-emerald-700 font-medium dark:text-emerald-400">원{pureCnt}</span>
                            <span>·</span>
                            <span className="text-sky-700 font-medium dark:text-sky-400">대{altCnt}</span>
                          </div>
                        </div>
                      </td>
                    </tr>
      ...
      {/* 5. [중요 개선] 하단 플로팅 맞비교 독 (모바일 하단 탭바 겹침 방지: bottom-16 sm:bottom-5) */}
      {compareList.length > 0 && (
        <div className="fixed bottom-16 sm:bottom-5 left-3 right-3 md:left-1/2 md:right-auto md:-translate-x-1/2 z-30 bg-slate-900 text-white px-3.5 py-2.5 rounded-2xl shadow-2xl border border-slate-700 flex items-center justify-between md:justify-start gap-2 sm:gap-3 animate-in fade-in slide-in-from-bottom-3 duration-200">
          ...
          <div className="flex items-center gap-1.5 text-xs truncate">
            {compareList.map((m) => (
              ...
              >
                {m.assemb_nm}
                <button onClick={() => toggleCompare(m)} className="text-slate-400 hover:text-white cursor-pointer">
            ...
            {compareList.length === 1 && (
              <button
                ...
                className="inline-flex items-center gap-1 px-2.5 py-1 bg-indigo-600/30 hover:bg-indigo-600/50 border border-indigo-400/40 rounded-lg text-indigo-300 font-medium text-xs whitespace-nowrap animate-pulse transition-colors cursor-pointer"
              >
