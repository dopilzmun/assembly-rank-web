...
import { MapPin, ArrowRight, Settings2, ChevronRight } from "lucide-react";
...
export default function MyDistrictWidget({ allMembers, onSelectMember }: MyDistrictWidgetProps) {
  ...
  return (
    <>
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-4 sm:p-5 flex flex-col justify-between space-y-3">
        ...
        {(isEditing || !district) && (
          <form onSubmit={handleSave} className="flex gap-2">
            <input
              ...
              className="flex-1 px-3 py-2 text-xs border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500 font-medium"
            />
            <button
              type="submit"
              className="px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-bold rounded-xl shadow-xs transition-colors cursor-pointer shrink-0"
            >
              저장
            </button>
          </form>
            ...
            {matchedMembers.length > 0 ? (
              matchedMembers.map((m) => (
                <div
                  ...
                  className="flex items-center justify-between p-3 rounded-xl bg-slate-50/80 hover:bg-indigo-50/50 border border-slate-100 hover:border-indigo-200 transition-all cursor-pointer group text-xs"
                >
                  <div className="flex items-center gap-2.5 truncate">
                    <span className="w-5 text-center font-mono font-bold text-indigo-600 shrink-0">
                      {m.rnkg ? `${m.rnkg}위` : "-"}
                    </span>
      ...
      {/* 홈 화면에서 즉시 열리는 의원 상세 성적표 Drawer */}
      <AssembDetailDrawer
